...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    // single product cart using template string 
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h6>Rating: ${product.rating.rate}/5.0</h6>
      <h6><i class="fas fa-user-check"></i> ${product.rating.count} people rated this</h6>
      ...
      `;
    // appends everything as a child of all-products to html 
...
// counting products and all functions called
let count = 0;
const addToCart = (id, price) => {
  ...
  // set inner text of count 
  document.getElementById("total-Products").innerText = count;
};

// floating to particular element using getInputValue function 
...
// Shooping price update function
...
const updateTaxAndCharge = () => {
  ...
  // condition of charge and tax for particular price  

  // <=200, no tax && charge 20
  if (priceConverted <= 200) {
    setInnerText("delivery-charge", 20);
  }

  // >200, *0.2 tax && charge 30
  if (priceConverted > 200) {
    setInnerText("delivery-charge", 30);
    setInnerText("total-tax", (priceConverted * 0.2).toFixed(2));
  }

  // >400, *0.3 tax && charge 50
  if (priceConverted > 400) {
    setInnerText("delivery-charge", 50);
    setInnerText("total-tax", (priceConverted * 0.3).toFixed(2));
  }

  // >500, *0.4 tax && charge 60
...
//grandTotal update function and showing total
const updateTotal = () => {
  const grandTotal =
...
// load all the products from api 
loadProducts();
